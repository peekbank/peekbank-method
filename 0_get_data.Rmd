...
saveRDS(all_aoi_timepoints, here("cached_intermediates","all_aoi.rds"))
```
...
```{r}
#store some dataset facts
peekbank_overall_properties <- tibble(
  n_subjects = unique(all_subjects$subject_id) %>% length(),
  n_datasets = unique(all_datasets$dataset_id) %>% length(),
  n_trials = unique(all_trials$trial_id) %>% length(),
  n_admins = unique(all_administrations$administration_id) %>% length(),
)
saveRDS(peekbank_overall_properties, here("cached_intermediates","peekbank_overall_properties.rds"))
```
...
* vanilla trial only
* age <=60 months inclusive 


```{r, eval=F}

all_aoi_timepoints <- readRDS(here("cached_intermediates","all_aoi.rds"))
...
```{r}
# how much do we expect from each

aoi_data_joined |> distinct(dataset_name, trial_id) |> group_by(dataset_name) |> tally()
```
...
#all_items |> left_join(included_items) |> filter(is.na(include))
...
dropping 16 garrison_bergelson trials (baba/wubby)
dropping 109 potter canine drink trials
...
confirm that we have expected about of changes and no more dropped trials 
...
subjects %>%
  ...
  unnest(cdi_responses) |> ungroup() |> distinct(language, instrument_type)
...
cdi_data <- cdi_data |> left_join(all_subjects |> select(subject_id, native_language))
