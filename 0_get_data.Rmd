...
```
35 datasets -- dataset list checks on based on what we'd expect from vanillaness checks + age (note that kartushina has only << 12 month olds and is thus excluded)
...
```{r}
# what we we losing
foo <- aoi_data_joined |> left_join(included_items, by = join_by(target_label == item)) |>
  left_join(included_items, by = join_by(distractor_label == item), 
            suffix = c("_target","_distractor")) 
```
...
```{r}
d_aoi |> distinct(dataset_name, trial_id) |> group_by(dataset_name) |> tally() |> rename(n_after=n) |> left_join(aoi_data_joined |> distinct(dataset_name, trial_id) |> group_by(dataset_name) |> tally()
)
```
