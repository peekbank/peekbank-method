...
```{r}
datasets <- d_aoi |>
  select(trial_id, dataset_name, administration_id, subject_id) |>
  distinct()
...
gb <- df |>
  ...
  filter(!is.na(accuracy)) |>
  prep_data_dim(administration_id, target_label, accuracy, repetition)
...
stuff <- d_aoi |>
  ...
  summarize(
    ...
    n_raters = n_distinct(target_label)
  ) |>
  left_join(readRDS(here("cached_intermediates/1_acc_icc_boot.rds"))) |>
  mutate(spread = upper - lower)

ggplot(stuff, aes(x = est, y = spread)) +
  geom_point()

ggplot(stuff |> filter(t_start == 200, t_end == 2000), aes(x = est, y = spread)) +
  geom_point()


ggplot(stuff, aes(x = n_raters, y = spread)) +
  geom_point()

ggplot(stuff, aes(x = n_admins, y = spread)) +
  geom_point()

ggplot(stuff |> filter(t_start == 200, t_end == 2000) |> filter(!is.na(spread)), aes(x = n_admins, y = spread, color = n_raters)) +
  geom_point() +
  scale_color_viridis()
...
df |>
  left_join(readRDS(here("cached_intermediates/1_acc_icc_boot.rds"))) |>
  mutate(spread = upper - lower) |>
  ggplot(aes(x = sdv, y = spread, color = count)) +
  geom_point() +
  coord_flip() +
  scale_color_viridis()
```
...
filter_icc <- function(data, col_est, col_upper, col_lower) {
  include <- data |>
...
a <-  readRDS(here("cached_intermediates/1_acc_icc_boot.rds")) |> filter_icc("est", "lower", "upper") |> mutate(name="ICC reliability") |> 
  mutate(combined=case_when(
    t_start==400 & t_end==3000 ~ "400-3000",
    t_start==200 & t_end==2000 ~ "200-2000",
    T ~ NA
  )) |> filter(!is.na(combined)) |> 
  select(dataset_name, est, combined, name, upper, lower) |> 
  rowwise() |> 
  mutate(lower=max(0, lower), 
         upper=min(1, upper)) |> 
  ungroup() |> 
  ggplot(aes(x=reorder(dataset_name, est), y=est))+geom_line() + geom_point(aes(color=combined))+coord_flip()+labs(x="dataset", y="Correlation")+scale_color_solarized(accent="red")+facet_wrap(~name, nrow=1)

b <-  readRDS(here("cached_intermediates/1_acc_cdi_boot.rds")) |> filter(!is.na(comp_est)) |> rename(est=comp_est) |> mutate(name="Comp CDI") |> 
  mutate(combined=case_when(
    t_start==400 & t_end==3000 ~ "400-3000",
    t_start==200 & t_end==2000 ~ "200-2000",
    T ~ NA
  )) |> filter(!is.na(combined)) |> 
  select(dataset_name, est, combined, name) |> 
  ungroup() |> 
  ggplot(aes(x=reorder(dataset_name, est), y=est))+geom_line() + geom_point(aes(color=combined))+coord_flip()+labs(x="dataset", y="Correlation")+scale_color_solarized(accent="red")+facet_wrap(~name, nrow=1)


c <-  readRDS(here("cached_intermediates/1_acc_cdi_boot.rds")) |> filter(!is.na(prod_est)) |> rename(est=prod_est) |> mutate(name="Prod CDI") |> 
  mutate(combined=case_when(
    t_start==400 & t_end==3000 ~ "400-3000",
    t_start==200 & t_end==2000 ~ "200-2000",
    T ~ NA
...
d <-  readRDS(here("cached_intermediates/1_acc_test_retest_boot.rds")) |> filter(!is.na(est)) |> mutate(name="Test retest") |> 
  mutate(combined=case_when(
    t_start==400 & t_end==3000 ~ "400-3000",
    t_start==200 & t_end==2000 ~ "200-2000",
    T ~ NA
...
icc_data <- readRDS(here("cached_intermediates/1_acc_icc_boot.rds")) |>
  filter(!is.na(est)) |>
  left_join(dataset_summ |> group_by(dataset_name) |>
    summarize(n_admins = n_distinct(administration_id))) |>
  mutate(
    range = str_c(t_start, "_", t_end) |> as.factor(),
    range = relevel(range, "400_3000")
  )

mod_icc <- lmer(est ~ range + (1 | dataset_name), data = icc_data, weights = n_admins)

a <- tidy(mod_icc, conf.int = T) |>
  filter(str_detect(term, "range")) |>
  mutate(start = str_sub(term, 6, -6) |> as.numeric(), end = str_sub(term, -4, -1)) |>
  ggplot(aes(x = start, y = estimate, ymin = conf.low, ymax = conf.high)) +
  facet_wrap(~end) +
  coord_flip() +
  geom_pointrange() +
  geom_hline(yintercept = 0)+labs(y="ICC")+  scale_y_continuous(breaks=c( -.1,  0, .1), limits=c(-.2, .2))



comp_data <- readRDS(here("cached_intermediates/1_acc_cdi_boot.rds")) |>
  filter(!is.na(comp_est)) |>
  left_join(dataset_summ |> group_by(dataset_name) |> filter(!is.na(comp)) |>
    summarize(n_admins = n_distinct(administration_id))) |>
  mutate(
    range = str_c(t_start, "_", t_end) |> as.factor(),
    range = relevel(range, "400_3000")
...
mod_comp <- lmer(comp_est ~ range + (1 | dataset_name), data = comp_data, weights = n_admins)

b <- tidy(mod_comp, conf.int = T) |>
  filter(str_detect(term, "range")) |>
  mutate(start = str_sub(term, 6, -6) |> as.numeric(), end = str_sub(term, -4, -1)) |>
  ggplot(aes(x = start, y = estimate, ymin = conf.low, ymax = conf.high)) +
  facet_wrap(~end) +
  coord_flip() +
  geom_pointrange() +
  geom_hline(yintercept = 0)+labs(y="CDI Comp")+  scale_y_continuous(breaks=c( -.1,  0, .1), limits=c(-.2, .2))


prod_data <- readRDS(here("cached_intermediates/1_acc_cdi_boot.rds")) |>
  filter(!is.na(prod_est)) |>
  left_join(dataset_summ |> group_by(dataset_name) |> filter(!is.na(prod)) |>
    summarize(n_admins = n_distinct(administration_id))) |>
  mutate(
    range = str_c(t_start, "_", t_end) |> as.factor(),
    range = relevel(range, "400_3000")
...
mod_prod <- lmer(prod_est ~ range + (1 | dataset_name), data = prod_data, weights = n_admins)

c <- tidy(mod_prod, conf.int = T) |>
  filter(str_detect(term, "range")) |>
  mutate(start = str_sub(term, 6, -6) |> as.numeric(), end = str_sub(term, -4, -1)) |>
  ggplot(aes(x = start, y = estimate, ymin = conf.low, ymax = conf.high)) +
  facet_wrap(~end) +
  coord_flip() +
  geom_pointrange() +
  geom_hline(yintercept = 0)+labs(y="CDI Prod")+  scale_y_continuous(breaks=c( -.1,  0, .1), limits=c(-.2, .2))



trt_data <- readRDS(here("cached_intermediates/1_acc_test_retest_boot.rds")) |>
  filter(!is.na(est)) |>
  left_join(dataset_summ |> make_test_retest_pairs() |> group_by(dataset_name) |>
...
mod_trt <- lmer(est ~ range + (1 | dataset_name), data = trt_data, weights = n_admins)

d <- tidy(mod_trt, conf.int = T) |>
  filter(str_detect(term, "range")) |>
  mutate(start = str_sub(term, 6, -6) |> as.numeric(), end = str_sub(term, -4, -1)) |>
  ggplot(aes(x = start, y = estimate, ymin = conf.low, ymax = conf.high)) +
  facet_wrap(~end) +
  coord_flip() +
  geom_pointrange() +
  geom_hline(yintercept = 0)+labs(y="Test retest")+
...
filter_icc <- function(data, col_est, col_upper, col_lower) {
  include <- data |>
    group_by(dataset_name, across(any_of("age_bin"))) |>
    summarize(
      nas = sum(is.na(.data[[col_upper]]) | is.na(.data[[col_lower]])),
      pct_0 = sum(.data[[col_est]] == 0) / n()
    )

  View(include)
  data |> semi_join(include |> filter(nas == 0, pct_0 < .5))
}
...
baseline_range <- "400_3000"

readRDS(here("cached_intermediates/1_acc_icc_boot.rds")) |>
  filter_icc("est", "lower", "upper") |>
  mutate(
    range = str_c(t_start, "_", t_end) |> as.factor(),
    range = relevel(range, baseline_range)
  ) |>
  rename(option = range) |>
  do_meta_comparison("est", "lower", "upper") |>
  filter(coef != "intrcpt") |>
  mutate(coef = str_replace(coef, fixed("d[[predictor]]"), "")) |>
  separate(coef, into = c("t_start", "t_end"), sep = "_") |>
  mutate(t_start = as.numeric(t_start)) |>
  ggplot(aes(x = t_start, y = estimate, ymin = ci.lb, ymax = ci.ub)) +
  geom_pointrange() +
  facet_wrap(~t_end) +
...
d_aoi |>
  filter(!is.na(correct)) |>
  make_age_bins() |>
  ggplot(aes(x = age_bin, fill = dataset_name)) +
...
long_datasets <- d_aoi |>
  group_by(dataset_name) |>
  summarize(min_t_norm = min(t_norm), max_t_norm = max(t_norm), count_0 = sum(t_norm == 0), count_2000 = sum(t_norm == 2000), count_3800 = sum(t_norm == 3800)) |>
  filter(min_t_norm <= 0, max_t_norm >= 2000) |>
  filter(count_0 / count_2000 > .8) |>
  filter(count_3800 / count_2000 > .8)
...
icc_noboot |>
  semi_join(long_datasets) |>
  group_by(t_start, t_end, dataset_name) |>
  summarize(
    N = n(),
    mean_icc = mean(icc, na.rm = TRUE)
  ) |>
  group_by(dataset_name) |>
  mutate(
    relative = mean_icc / max(mean_icc),
...
acc_boot_summary_long <- acc_boot |>
  semi_join(long_datasets) |>
  do_meta(c("t_start", "t_end"))
```
...
accs_cdi_summ_long <- accs_cdi |>
  semi_join(long_datasets) |>
  do_cdi_meta(c("t_start", "t_end"))
```
...
```{r}
accs_cdi_summ |> ggplot(aes(x = t_start, y = prod_estimate, ymin = prod_ci.lb, ymax = prod_ci.ub, color = as.character(t_end))) +
  geom_pointrange(position = position_dodge(width = 50)) +
  coord_flip()

accs_cdi_summ_long |> ggplot(aes(x = t_start, y = prod_estimate, ymin = prod_ci.lb, ymax = prod_ci.ub, color = as.character(t_end))) +
  geom_pointrange(position = position_dodge(width = 50)) +
  coord_flip()
```
...
```{r}
accs_cdi_summ |> ggplot(aes(x = t_start, y = comp_estimate, ymin = comp_ci.lb, ymax = comp_ci.ub, color = as.character(t_end))) +
  geom_pointrange(position = position_dodge(width = 50)) +
  coord_flip()

accs_cdi_summ_long |> ggplot(aes(x = t_start, y = comp_estimate, ymin = comp_ci.lb, ymax = comp_ci.ub, color = as.character(t_end))) +
  geom_pointrange(position = position_dodge(width = 50)) +
  coord_flip()
```
...
```{r}
accs_cdi_summ |> ggplot(aes(x = t_start, y = age_estimate, ymin = age_ci.lb, ymax = age_ci.ub, color = as.character(t_end))) +
  geom_pointrange(position = position_dodge(width = 50)) +
  coord_flip()

accs_cdi_summ_long |> ggplot(aes(x = t_start, y = age_estimate, ymin = age_ci.lb, ymax = age_ci.ub, color = as.character(t_end))) +
  geom_pointrange(position = position_dodge(width = 50)) +
  coord_flip()
```
...
```{r}
accs_cdi_summ_age |>
  filter(!is.na(prod_estimate)) |>
  ggplot(aes(x = t_start, y = prod_estimate, ymin = prod_ci.lb, ymax = prod_ci.ub, color = as.character(t_end))) +
  geom_pointrange(position = position_dodge(width = 50)) +
  coord_flip() +
  facet_wrap(~age_bin)
```
...
```{r}
accs_cdi_summ_age |>
  filter(!is.na(comp_estimate)) |>
  ggplot(aes(x = t_start, y = comp_estimate, ymin = comp_ci.lb, ymax = comp_ci.ub, color = as.character(t_end))) +
  geom_pointrange(position = position_dodge(width = 50)) +
  coord_flip() +
  facet_wrap(~age_bin)
```
...
```{r}
accs_cdi_summ_age |> ggplot(aes(x = t_start, y = age_estimate, ymin = age_ci.lb, ymax = age_ci.ub, color = as.character(t_end))) +
  geom_pointrange(position = position_dodge(width = 50)) +
  coord_flip() +
  facet_wrap(~age_bin)
```
...
accs_cdi_byage |>
  ...
  ggplot(aes(x = t_start, y = age_est, ymin = age_lower, ymax = age_upper, color = as.character(t_end))) +
  geom_pointrange(position = position_dodge(width = 50)) +
  coord_flip() +
  facet_wrap(~dataset_name)
...
accs_cdi_byage |>
  filter(age_bin == "<18") |>
  filter(!is.na(prod_est)) |>
  ggplot(aes(x = t_start, y = prod_est, ymin = prod_lower, ymax = prod_upper, color = as.character(t_end))) +
  geom_pointrange(position = position_dodge(width = 50)) +
  coord_flip() +
  facet_wrap(~dataset_name)
...
accs_cdi_byage |>
  filter(age_bin == "<18") |>
  filter(!is.na(comp_est)) |>
  ggplot(aes(x = t_start, y = comp_est, ymin = comp_lower, ymax = comp_upper, color = as.character(t_end))) +
  geom_pointrange(position = position_dodge(width = 50)) +
  coord_flip() +
  facet_wrap(~dataset_name)
```
...
```{r}
accs_kid_test_retest |> ggplot(aes(x = t_start, y = est, ymin = lower, ymax = upper, color = as.character(t_end))) +
  geom_pointrange(position = position_dodge(width = 50)) +
  facet_wrap(~dataset_name) +
  geom_hline(yintercept = 0)
```
...
```{r}
accs_kid_test_retest_summ |> ggplot(aes(x = t_start, y = estimate, ymin = ci.lb, ymax = ci.ub, color = as.character(t_end))) +
  geom_pointrange(position = position_dodge(width = 50)) +
  geom_hline(yintercept = 0)
```
...
d_aoi |>
  group_by(dataset_name, t_norm) |>
  mutate(has_data = aoi %in% c("target", "distractor")) |>
  summarize(
    s = sum(has_data),
    n = n()
  ) |>
  ggplot(aes(x = t_norm, y = n)) +
  geom_col(fill = "blue") +
  facet_wrap(~dataset_name, scales = "free_y") +
  coord_cartesian(xlim = c(0, 4050)) +
...
d_aoi |>
  ...
  group_by(dataset_name) |>
  mutate(max_n = max(n)) |>
  ggplot(aes(x = t_norm, y = s / n, color = dataset_name)) +
  geom_line() +
  facet_wrap(~dataset_name) +
  coord_cartesian(xlim = c(-4000, 4050), ylim = c(0, 1)) +
  theme(legend.position = "none") +
...
d_aoi |>
  ...
  group_by(dataset_name) |>
  mutate(max_n = max(n)) |>
  ggplot(aes(x = t_norm, y = s, color = dataset_name)) +
  geom_line() +
  facet_wrap(~dataset_name, scales = "free_y") +
...
d_aoi |>
  ...
  scale_fill_manual(aesthetics = c("fill", "color"), values = c("target" = "darkgreen", "distractor" = "red", "missing" = "grey", "other" = "grey")) +
  theme(legend.position = "bottom")
...
by_age |>
  ...
  ggplot(aes(x = t_norm, y = n, fill = aoi, color = aoi)) +
  geom_col(position = "stack") +
  facet_wrap(age_bin ~ dataset_name, scales = "free_y") +
  coord_cartesian(xlim = c(-4000, 4050)) +
  geom_vline(xintercept = 0) +
  geom_vline(xintercept = 2000) +
  geom_vline(xintercept = 3000) +
