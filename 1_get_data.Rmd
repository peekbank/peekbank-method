...
d_trial <- aoi_data_joined |>
  filter(!(dataset_name %in% c("casillas_tseltal_2015", 
                               ...
                               "weisleder_stl",
                               "xsectional_2007", 
                               "garrison_bergelson_2020"))) |> # until renaming issue is fixed
...
```{r}
all_items <- tibble(item = sort(unique(c(d_trial$target_label, d_trial$target_label))))
...
d_trial <- d_trial |>
  mutate(target_label = stringr::str_to_lower(target_label), 
         distractor_label = stringr::str_to_lower(distractor_label)) |> 
         filter(!(target_label %in% 
