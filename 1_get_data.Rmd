...
author: "Mike"
date: "2/19/2021, updated 4/15/25"
output: 
  html_document:
    toc: true
    toc_depth: 2
...
```{r}
all_aoi_timepoints <- lazy_dt(all_aoi_timepoints)

all_administrations <- all_administrations |> select(administration_id, age, dataset_id, subject_id, dataset_name) |> filter(age <= 60)

all_trials <- all_trials |> select(trial_id, trial_order, trial_type_id, dataset_id, dataset_name)

all_trial_types <- all_trial_types |> select(trial_type_id, dataset_id, dataset_name, distractor_id, target_id, target_side, vanilla_trial) |> filter(vanilla_trial == TRUE)
...
aoi_data_joined <- all_aoi_timepoints |>
  ...
  left_join(all_stimuli %>%
    ...
    select(dataset_name, subject_id, administration_id, trial_id, trial_order,
         dataset_id, target_id, distractor_id, t_norm, age, aoi, target_label, distractor_label, target_side) |> 
  as_tibble()

saveRDS(aoi_data_joined, file= here("cached_intermediates","1_aoi_data_joined.Rds"))
```
...
TODO CHECK: note that newman_genderdistractor is not vanilla for condition reasons. 
...
aoi_data_joined <- aoi_data_joined |> lazy_dt()
...
```{r}
included_items <- read_csv(here("metadata","included_items_2025.1.csv")) |> select(-item.2024)
...
saveRDS(d_aoi, file = here("cached_intermediates","1_d_aoi.Rds"))
```

## Distractor label version

For the stimulus repetition anlaysis, we want just just distractor label, but also distractor id  which wasn't in the d_aoi

```{r}
#distractor_id <- aoi_data_joined |> select(trial_id, distractor_id, distractor_label) |> distinct()

#d_aoi_distractor_id <- readRDS(here("cached_intermediates","1_d_aoi.Rds")) |> left_join(distractor_id)

saveRDS(d_aoi_d, here("cached_intermediates", "1_d_aoi_distractor.rds"))
```

```{r}
d_aoi <- readRDS(here("cached_intermediates","1_d_aoi.Rds")) 
d_aoi_d <- readRDS(here("cached_intermediates", "1_d_aoi_distractor.rds"))

d_aoi_d <- d_aoi_d |>   mutate(correct = ifelse(aoi == "target", 1, 
                          ifelse(aoi == "distractor", 0, NA)))
```
...
saveRDS(cdi_data, file=here("cached_intermediates", "1_cdi_subjects.Rds"))
```
...
#   geom_point(alpha=.5) 
# >>>>>>> 9f838ecc75c2df3fb4ca309e808fd360af8a6724
```

# Units for ICC etc
Here we have everything by "dataset", but in some cases datasets may contain multiple different item-banks/studies.

things we can look for 
```{r}
d_aoi <- readRDS(here("cached_intermediates", "1_d_aoi.Rds"))

trials <- d_aoi |> distinct(dataset_name, administration_id, subject_id, age, trial_id, target_label, distractor_label)
```

```{r}
trials |> ggplot(aes(x=dataset_name, y=age))+geom_jitter(alpha=.1)+coord_flip()
```
```{r}
 plots <- trials |> distinct(dataset_name) |> pull(dataset_name) |> map( \(d) {
   trials |> filter(dataset_name==d) |> ggplot(aes(x=target_label, y=reorder(administration_id, age)))+geom_jitter(width=.2,height=.1, alpha=.5)+coord_flip()+labs(title=d)
 })
...
heterogenous:
* perry_cowpig: either kids see 1 set of 3 pairs (6 items) or a different set of 3 pairs (6 items)
* pomper_saffran_2016 -- something complicated where item co-occurrance isn't fully random, but not fully split into lists either
*adams-marchman -- has 3 age groups with 2 lists/age -- 4 different list options, with some item overlaps between them
* fernald totlot -- looks like 4 age groups, a different list option for each age group, some overlaps across them 
* sander-montant -- 2 age groups with slightly overlapping lists
* fmw -- 2 age groups , mostly overlapping lists
* pomper_prime -- 2 sets of 2 item lists that also cross
* potter_canine -- 2 mostly distinct lists
* fernald marchman2012 -- 3 age mins with somewhat overlapping lists
* garrison_bergelson -- just chaos

Given this, I don't think it makes sense to make changes. We'll leave it as ICC/dataset. 
