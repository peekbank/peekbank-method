...
bc_boot_summ <- boot_graph |>
  ...
  group_by(b_start, b_end, t_start, t_end) |>
  nest() |>
...
no_bc_boot_age <- readRDS(here("cached_intermediates", "1_acc_icc_boot_byage.rds")) |> filter(t_start == 500)
```
...
bc_boot_age_summ <- boot_graph_age |>
  ...
  group_by(b_start, b_end, age_bin, t_start, t_end) |>
  nest() |>
  mutate(icc = map(data, \(d){
    rma(d$est, d$var) |>
      summary() |>
...
```{r}
bc_accs_kid_cdi_summ |> ggplot(aes(x = b_start, y = age_estimate, ymin=age_ci.lb, ymax=age_ci.ub, color=as.character(b_end))) +geom_pointrange(position=position_dodge(width=.5))+coord_flip()+facet_wrap(~t_end)
