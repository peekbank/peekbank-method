...
datasets <- d_aoi |> select(trial_id, dataset_name, administration_id, subject_id) |> distinct() |> 
  filter(!dataset_name%in% c("newman_genderdistractor"))
...
datasets |> left_join(cdi_data) |> distinct(administration_id, prod, comp) |> filter(!is.na(prod))

datasets |> left_join(cdi_data) |> filter(!is.na(prod))

datasets |> left_join(cdi_data) |> filter(!is.na(comp))
```

how much data do we have?

```{r}
# look at some individual trials

d_aoi |> filter(dataset_name=="frank_tablet_2016") |> filter(target_label=="dog") |> distinct(distractor_label)


relevant <- d_aoi |> filter(dataset_name=="frank_tablet_2016") |> filter(target_label=="dog")|>
  filter(trial_id %in% c(
    124998, 124182, 124589, 123365, 124158, 124301)) |>
  mutate(aoi= ifelse(aoi %in% c("target", "distractor"), aoi, "other/missing")) 

relevant |> 
  ggplot(aes(x=t_norm, y=aoi, color=as.factor(trial_id)))+geom_line(aes(group=1), color="grey")+geom_point()+
  geom_point(data=relevant |> filter(aoi=="other/missing"), color="grey")+facet_wrap(~trial_id)+
  geom_vline(xintercept=0)+
  theme_bw()+
  theme(legend.position = "none", strip.text = element_blank())

d_aoi |> filter(dataset_name=="frank_tablet_2016") |> filter(target_label=="dog")|> filter(trial_id==124806) |> ggplot(aes(x=t_norm, y=aoi))+geom_point()+geom_line(aes(group=1))
