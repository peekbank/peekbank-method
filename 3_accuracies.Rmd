...
writeRDS(file = here("cached_intermediates", "3_accs.Rds"), accs)
```
...
looks like long window is best, but when exactly is the best start point?
...
saveRDS(accs, here("cached_intermediates", "2_iccs_narrow.Rds"))
```
...
```{r}
accs_byage <- readRDS(here("cached_intermediates", "2_iccs_narrow.Rds"))
accs_byage_summary <- accs_byage |>
  group_by(younger, t_start, t_end, object) |>
  summarize(
    N = n(),
...
accs_byage_summary |>
  ...
  knitr::kable()
```
My bet is that these ICC differences mostly don't matter;  we're probably seeing that anything starting in the 350-500 range is fine. (also orderings for younger and older kids are remarkably similar, even as their overall iccs are different, probably from different datasets)
...
```{r}
load(file = here("cached_intermediates", "3_cdi_corrs.Rds"))
...
```{r}
ggplot(
  filter(cdi_corrs_long, !is.na(r), dataset_name == "adams_marchman_2018"),
  aes(col = r)
) +
  geom_vline(xintercept = 0, linetype = "dashed") +
  geom_hline(yintercept = 0.05, lty = 3) +
...
save(file = here("cached_intermediates", "3_cdi_corrs_alldata.Rds"), cdi_corrs_alldata)
```

```{r}
load(file = here("cached_intermediates", "3_cdi_corrs_alldata.Rds"))
