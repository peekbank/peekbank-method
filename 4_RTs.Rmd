...
```
Notes from VB 8/26 on combinatorial things to try:

* launch vs land RT
* raw vs log 
* filter by max shift length (x different times)
* filter min start time
* do we exclude based on pre-min loss (between t_0 and min start time)
* do we count initiation as first time you leave or last time you leave (before landing)
...
d_rt |>
  ...
  mutate(
    initiation_type = shift_start_rt != last_shift_rt,
    long_shift = shift_length > 600
  ) |>
  group_by(initiation_type, long_shift) |>
  tally()
```
...
combinatorics:
* land / first launch / last launch
* raw /log
* filter shift_length <=600 or no
...
rt_iccs <- d_rt_dt |>
  ...
  mutate(icc_admin = map(data, \(d) {
    ...
    tibble(measure = rt_names, icc = icc_values)
  })) |>
  select(-data) |>
  unnest(icc_admin)
```
TODO figure out what's going on with ICC's of 0 and also the NaNs!
```{r}
```
...
rt_iccs_coded <- rt_iccs |> mutate(
  ...
  measure = str_replace_all(measure, "first_laumch_rt", "") |>
    str_replace_all("last_launch_rt", "") |>
    str_replace_all("land_rt", "")
)
...
rt_iccs_coded |>
  ...
  summarize(mean_icc = mean(icc)) |> View()
```
