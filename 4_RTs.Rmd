...
d_rt_more <- rle_data %>%
  group_by(administration_id, trial_id, trial_order) %>%
  nest() |>
  expand_grid(window = c(0, 200, 400), time_0 = c(T, F), time_end = c(T, F), during = c(T, F)) %>%
  head() |>
  mutate(rts = pmap(list(data, time_0, window, time_end, during), ~ get_rt(data, t_0 = time_0, window_length = window, t_end = time_end, window_mostly_region = during))) |>
  unnest(cols = c(rts))

saveRDS(file = here("cached_intermediates", "4_d_rt_more.Rds"), d_rt_more)

d_rt_more[1, ] %>%
  mutate(data = pmap(., ~ get_rt(data[[1]], t_0 = time_0, window_length = window, t_end = time_end, window_mostly_region = during))) |>
  View()
```
