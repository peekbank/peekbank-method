...
saveRDS(bootstrap_cdi, here("cached_intermediates", "4_rt_cdi_boot.rds"))
...
bootstrap_cdi |>filter(window==400) |> filter(!is.na(cor_prod)) |>  ggplot(aes(x = str_c(approach, "_", logged), y = cor_prod, ymin=cor_prod_lower, ymax=cor_prod_upper,  col=approach)) +
  geom_pointrange() +
  coord_flip() +
...
bootstrap_cdi |>filter(window==400) |> filter(!is.na(cor_comp)) |>  ggplot(aes(x = str_c(approach, "_", logged), y = cor_comp, ymin=cor_comp_lower, ymax=cor_comp_upper,  col=approach)) +
  geom_pointrange() +
  coord_flip() +
  theme(legend.position = "none") +
  facet_wrap(~dataset_name)
...
bootstrap_cdi |>filter(window==400) |> filter(!is.na(cor_age)) |>  ggplot(aes(x = str_c(approach, "_", logged), y = cor_age, ymin=cor_age_lower, ymax=cor_age_upper,  col=approach)) +
  geom_pointrange() +
  coord_flip() +
  theme(legend.position = "none") +
  facet_wrap(~dataset_name)
