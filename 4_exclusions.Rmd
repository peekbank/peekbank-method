...
trial_test_retest_summ <-  trial_test_retest |>do_test_retest_meta(c("t_end", "exclude_less_than", "look_both"))
```


```{r}
trial_test_retest |> ggplot(aes(x=exclude_less_than, y=est, ymin=lower, ymax=upper, color=look_both))+geom_pointrange(position = position_dodge(width=.05))+facet_wrap(t_end~dataset_name)+geom_hline(yintercept=0)
```


```{r}
trial_test_retest_summ |> ggplot(aes(x=exclude_less_than, y=estimate, ymin=ci.lb, ymax=ci.ub, color=look_both))+geom_pointrange(position = position_dodge(width=.05))+geom_hline(yintercept=0)+facet_wrap(~t_end)
...
# Summary
Excluding trials for less than 50% looks or not looking (ever) at each of D and T are reasonable; but don't seem to have large gains over not doing any exclusions. 
