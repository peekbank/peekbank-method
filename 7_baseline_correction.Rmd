...
d_summary_cutoffs <- map(baseline_cutoffs, function(baseline_cutoff) {
  d_trial |>
    group_by(dataset_name, trial_id, subject_id, administration_id, 
             target_label) |>
    summarise(baseline = mean(correct[t_norm < baseline_cutoff], na.rm=TRUE),
              accuracy = mean(correct[t_norm > 500], na.rm=TRUE), 
              bc_accuracy = accuracy - baseline, 
              target = sum(correct[t_norm > 500], na.rm=TRUE),
...
iccs_long_cutoffs <- iccs_cutoffs |>
  ...
  separate_wider_delim(measure, delim = "_", 
...
iccs_long_cutoffs |> 
  ggplot(aes(x = baseline_cutoff, y = icc, 
  ...
  labs(x = "Baseline window cutoff (ms)",
       y = "ICC",
       col = "Measure",
