...
We keep with the datasets with trials with at least 2 repetitions

```{r}

d_trial_summary %>% 
  filter(max_stimulus_pair_instance >=2) %>% 
  ggplot(aes(x = stimulus_pair_instance)) + 
  geom_histogram() + 
...
Select two or three datasets and see if this make sense
...
repetitions_data_long%>% 
  ...
  mutate(max_age = max(age),
         min_age = min(age),.by = c(age_bin)) %>% 
  mutate(age_bin_limits = paste(min_age, "-", max_age)) %>% 
...
See what happens with age
See if we can expand to three
...
rep_timecourse <- d_trial %>%
  filter(dataset_name != "perry_cowpig") %>% 
  filter(dataset_name != "swingley_aslin_2002") %>%  
  left_join(repetitions_data) %>% 
  filter(!is.na(type_of_instance)) %>% 
  filter(age >= 12) %>%  
...
# 3. Guessing-error index

I wonder if we could calculate an index of errors related to the order/guessing effect. I should be pre-accuracy in "Not repeated" minus pre-accuracy in "Repeated pair".

This index could be used for correlation analysis with other variables (e.g. age)

```{r}


guessing_index_data = repetitions_data %>% 
  filter(type_of_instance != "Repeated pair and target") %>% 
  group_by(dataset_name, administration_id, type_of_instance, age) %>% 
  summarise(mean_pre_accuracy = mean(pre_accuracy, na.rm = T)) %>% 
  
  
  pivot_wider(id_cols = c("dataset_name", "administration_id", "age"),
              names_from = type_of_instance, 
              values_from = mean_pre_accuracy) %>% 
  janitor::clean_names() %>% 
  mutate(guessing_index = repeated_pair) %>% 
  select(dataset_name, administration_id, age, guessing_index)



guessing_index_data %>%
  # filter(age <30) %>% 
  ggplot(aes(x = age, y = guessing_index))+
  geom_point()+
  geom_smooth(method = "lm") +
  facet_wrap(~dataset_name)
...
guessing_index_data %>%
  filter(age < 24) %>% 
  # filter(dataset_name == "yurovsky_2017") %>%
  # filter(age <30) %>% 
  ggplot(aes(x = age, y = guessing_index))+
  geom_point()+
  geom_smooth(method = "lm",) +
...
guessing_index_data %>%
  ...
  ggplot(aes(x = age, y = guessing_index))+
  geom_point(aes(color = dataset_name))+
  geom_smooth(method = "lm",)  +
  stat_cor()
...
repetitions_data %>%
  ...
  ggplot(aes(x = age, y = pre_accuracy,color = type_of_instance))+
  geom_point(alpha = 0.1)+
  # geom_line(aes(group = type_of_instance))+
  geom_smooth(method = "lm") +
  stat_cor()
...
```
yurovsky_2017 shows something interesting. Maybe we should think about how we pool age data across studies

plot with one line for non repeated repeated pair
