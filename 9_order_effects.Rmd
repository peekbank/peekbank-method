...
for_plot <- d_trial_summary |>
  select(dataset_name, target_label, distractor_label, stimulus_pair, max_stimulus_pair_instance, stimulus_pair_target_instance) |>
  group_by(dataset_name, target_label, distractor_label, stimulus_pair, max_stimulus_pair_instance) |>
  summarize(max_target_pair_instance = max(stimulus_pair_target_instance)) |>
  unique()

for_plot |> ggplot(aes(x = max_target_pair_instance, fill = dataset_name)) +
  geom_histogram()

for_plot |> ggplot(aes(x = max_stimulus_pair_instance, fill = dataset_name)) +
  geom_histogram()


for_plot |>
  select(dataset_name, stimulus_pair) |>
  unique() |>
  group_by(dataset_name) |>
  tally() |>
  arrange(desc(n))
# between 4 and 241 pairs; with most not having all that many
```
so repeating the same item is pretty common (probably in part to counterbalance side)

goal is to understand whether infant behavior to repeated stim pairs is somehow different
* for instance, does habituation/familiarity with the images matter
* do kids have a was-target preference 
--> this will lead to advice on when and how to repeat or not repeat items (possibly)
note however that the nth repetition of items is going to be confounded with things like how many trials the infant has seen, so analysis might get annoying

so, there's both looking at what the looking trajectories are (which people have analysed below)
and could also look at the effects on acc / RT / not having RT etc (from other processed data)

are there datasets where the second time a stimulus pair is shown, either target is same or target is different for the same pair? 

also a question of whether *exact* stimulus pair matters (same pictures), or whether this also occurs with different exemplars (hypothesis is that it would only happen for exact stimulus, but we have the data to check)

for the purpose of methods paper mostly want to check how RT and accuracy vary across 
* trial_num 
* repeated stim (x target)
because if they don't show effects on those measures, it's less of a concern. 

separately, pre-looking one sided would especially cause issues for baseline correction when there is counterbalancing
...
if we think trial order might matter with need to get distractor id
