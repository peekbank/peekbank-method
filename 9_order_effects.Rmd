...
hackathon_datasets <- c( 
"ronfard_2021"            ,"perry_cowpig",           
"pomper_saffran_2016"     ,"gazetriggered_2020" ,    
"adams_marchman_2018"     ,"baumgartner_2014"    ,   
"newman_genderdistractor" ,"pomper_salientme"     ,  
"fmw_2013"                ,"ferguson_eyetrackingr" , 
"yurovsky_2017"           ,"swingley_aslin_2002"    ,
"reflook_v4"              ,"pomper_prime"           ,
"potter_remix"            ,"weaver_zettersten_2024" ,
"potter_canine"           ,"mahr_coartic"           ,
"bacon_gendercues"        ,"frank_tablet_2016")

d_trial_summary <- d_aoi %>%
  # filter(dataset_name %in% hackathon_datasets) %>% 
...
We could focus in trials with stimulus_pair_instance == 1 \| 2 to make it | Stimulus_pair_instance | Stimulus_target_instance |                                                              |
|------------------------|--------------------------|--------------------------------------------------------------|
| 1                      | 1                        | 1st time pair and target are shown                           |
| 2                      | 1                        | 1st time target was shown even though pair was already shown |
| 2                      | 2                        | 2nd time target is shown along with pair                     |easier to approach. In that case, we should have these combinations
...
repetitions_data_long_summary = repetitions_data_long %>% 
  group_by(dataset_name, stimulus_pair_instance ,stimulus_pair_target_instance, type_of_instance) %>%
  summarize(n = n()) %>%
...
bad_n_datasets = repetitions_data_long_summary[repetitions_data_long_summary$proportion < .1,]$dataset_name
