...
bibliography      : ["peekbank-methods-ref.bib", "r-references.bib"]
...
Over the first few months of life, children become experts on their native language, organizing the phonological system of the speech that they hear and parsing speech into meaningful units.  A bit later, children begin to show early signs of comprehension of words or phrases through simple behaviors like turning their head when their name is called (e.g., “Hey, Billy!”) or by reaching for or pointing to an object that is named (e.g., “Where’s the ball?).  Documenting these everyday accomplishments marks important milestones in children’s development, yet such global assessments tell us little about the scope of children’s abilities and rely on particular behaviors (e.g., pointing) and a child who both understands and complies with instructions. Moreover, such global assessments gloss over the millisecond-level processes that take place during real-time language comprehension.  For example, decades of experimental work have shown us that children, like adults, process speech incrementally, as it unfolds in time (Jones & Rowland, 2017; Lew-Williams & Fernald, 2007), and use features of the speech signal to extract meaning that will support language comprehension (Law & Edwards, 2014; Fernald et al., 2016). These moment-to-moment processes would have been difficult to document without child-friendly procedures that allow precision in capturing how children’s behaviors are time-locked to speech. 

TODO could add some citations that early language is crucial to longer term well being. 

[LWL is a good method/measure for early language. -- low demands, wide age range, real time measure]. 

One procedure that is used to capture these moment-to-moment processes is the “looking-while-listening” (LWL) procedure, a low-demand, eye-tracking task that assesses children’s eye-movements as they listen to speech in real time. This method has been a key tool in propelling forward our understanding of infants’ language processing and knowledge. For example, studies have shown that, over the first few years of life, children become faster and more accurate in responding to familiar words (e.g., Fernald et al., 2006) and are increasingly able to distinguish between phonologically well-formed words versus words that are mispronounced (e.g., Swingley & Aslin, 1993). Moreover, children become increasingly more skilled at distinguishing pictures of familiar objects between categories, like body parts vs. foods, as well as more fine-grained distinctions within categories, like toes vs. feet (e.g., Bergelson et al. XXX).  Measures of children’s skill in the LWL task have also proven useful as an index of individual differences. For example, children who show more efficient language processing at 18 months show larger vocabulary size and more rapid vocabulary growth over the second and third years than children with less efficient processing (Fernald & Marchman, 2012; Fernald et al. 2006; Peter et al. 2019). Language processing efficiency has also been shown to link to later language, cognitive, and non-verbal outcomes in both typically-developing (Peter et al., 2019; Law & Edwards, 2014) and preterm children (Marchman et al., 2017, 2022). 

mention that LWL is widely used in various paradigms (both longitudinal indiv-differences & experimental paradigms)
...
[Measurement is important, but often is difficult with young children]
...
[how to frame this as building on / complementing past work. Past work on methods was more on experiment design & stimulus & more theory driven.]
One key reason for the success of this line of research, arguably, is the key attention paid to measurement in developing the looking-while-listening method. Past work has articulated best practices in collecting and analyzing looking-while-listening measures with a particular focus on stimulus and design choices that make looking preferences maximally informative about word recognition processes and innovations in the analysis of looking data that allow researchers to draw inferences about how infants’ language processing is unfolding over time [Fernald chapter; other core citations]. However, despite widespread use of the looking-while-listening method in language development research, there is still tremendous variation and limited consensus on how to best process and analyze infant looking time data. One key reason for the lack of widespread consensus is that infant recruitment is time- and cost-intensive and datasets collected by individual labs are often small. This makes it difficult for any one researcher to systematically evaluate the consequences of specific analytic choices independently of the data collected for a particular study. Instead, researchers are often forced to make ad-hoc decisions or to adopt idiosyncratic lab practices that themselves originate from ad-hoc subjective judgments of what “seems to work”. How can we put the measurement and analysis of infant word recognition and online language processing on firmer empirical footing?
...
From a psychometric perspective, the goal of these analyses should be to distill the best possible measures from the rich time course of behavior in the looking-while-listening task. Ideally, these measures should be reliable, yielding the maximal signal about individual children relative to both measurement error and irrelevant artifacts. Further, they should be valid, meaning that it relates maximally to other measures that reflect related constructs (for example, measures of vocabulary size or language processing ability). 

[sketch high-level approach: Peekbank]
Peekbank offers a solution to this core issue by providing a large database of infant eye-tracking data from the looking-while-listening paradigm. Peekbank currently houses XX datasets from infants across a wide age range [xx - xx] and across a broad range of test words and materials, resulting in observations from over XXX infants participating in the looking-while-listening task. The Peekbank database is represented in a unified format to ease cross-dataset analyses and comparisons. The Peekbank database offers infant researchers for the first time a database large enough to systematically test the measurement properties that result from a variety of common analytic decisions. By evaluating these common decisions in a bottom-up, data-driven way, we can establish best practices in measuring infant word recognition.

[scope]
In the current work, While LWL studies and peekbank are used for a variety of conditions, here we focus only on items that test vocabulary directly with familiar targets, familiar distractors, and no informative carrier phrases or experimental manipulations. These items are the ones where there is the most conceptual clarity on what reliability and validity would mean since they are expected to reflect the construct of word knowledge and correlate with other measures of word knowledge. We have over XXX trials of this type in wordbank.  To measure reliability, we consider both intraclass correlation measuring inter-item reliability and test-retest reliability for administrations with the same child at close intervals. For validity, we compare to sumscores on another common assessment of children's early vocabulary, the MBCDI. [possibly citations on how CDI-LWL correlations are not as good as one would hope for... especially for item-level correlations @lopezperez2025]

We provide guidance on how to process LWL data (and design LWL studies) to maximize the reliability and validity of the resultant measures, based on a data-driven exploration of different options in peekbank. To foreshadow our results, we found that many common practices provide reasonable reliability and validity, but are not optimized to squeeze the most reliability out of the data. 
<!--
...
To measure reliability, we use both within-dataset inter-item reliability (ICC) and test-retest reliability within datsets with multiple LWL sessions close in time for the same children. For validity, we look at the correlations between LWL measures and parent reports of vocabulary measured with MBCDI from around the same time as a measure of external validity.  
