...
<!-- VB comment: attempts to identify the following citations has not succeeded
fernald2016
law2014
...
## Recommendation 1: Measure accuracy over a long time window, such as from 400 ms - 3000 ms after the point of disambiguation.
...
[method/results]
We calculated the reliabilities for accuracy calculated across a range of different window onsets and offsets (\ref{fig:acc-heat-map}). As expected, including time points from before the point of disambiguation decreases reliability. Our initial exploration found a region of high reliability with onsets in the 600-800 range and offsets in the TODO range. 
...
```{r acc-heat-map, fig.cap="TODO heat map"}
...
acc_all <- acc_icc |>
  ...
  bind_rows(acc_cdi |> rename(estimate = prod_estimate, ci.lb = prod_ci.lb, ci.ub = prod_ci.ub) |> mutate(Type = "Corr. with CDI Production")) |>
  bind_rows(acc_cdi |> rename(estimate = comp_estimate, ci.lb = comp_ci.lb, ci.ub = comp_ci.ub) |> mutate(Type = "Corr. with CDI Comp.")) |>
  bind_rows(acc_test_retest |> mutate(Type = "Test-retest reliability"))
```
...
bc_acc_all <- bc_icc |>
  mutate(Type = "ICC reliability") |>
  bind_rows(bc_cdi|> rename(estimate = prod_estimate, ci.lb = prod_ci.lb, ci.ub = prod_ci.ub) |> mutate(Type = "Corr. with CDI Prod.")) |>
  bind_rows(bc_cdi |> rename(estimate = comp_estimate, ci.lb = comp_ci.lb, ci.ub = comp_ci.ub) |> mutate(Type = "Corr. with CDI Comp.")) |>
  bind_rows(bc_test_retest |> mutate(Type = "Test-retest reliability")) |>
  mutate(
