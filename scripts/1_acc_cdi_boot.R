...
acc_cdi_age_split <- function(t_start = -500, t_end = 4000) {
  d_aoi_age_split |>
    filter(t_norm > t_start, t_norm < t_end) |>
    group_by(dataset_name, dataset_id, age_bin, administration_id, target_label, trial_id) |>
    summarise(
...
accs_boot_cdi_byage_split <- acc_params |>
  mutate(summary_data = pmap(list(t_start, t_end), \(t_s, t_e) acc_cdi_age_split(t_s, t_e))) |>
  partition(cluster) |>
  mutate(cdi = map(summary_data, \(d) boot_cdi(d, by_age = TRUE))) |>
  collect() |>
  select(-summary_data) |>
...
saveRDS(accs_boot_cdi_byage_split, "../cached_intermediates/1_acc_cdi_boot_byage_split.rds")
