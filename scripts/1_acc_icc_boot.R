...
d_aoi_age_split <- make_age_bins_2(d_aoi)
...
accs_summarized_age_split <- acc_params |>
  mutate(summary_data = pmap(list(t_start, t_end), \(t_s, t_e) summarize_accuracy(d_aoi_age_split, t_s, t_e)))
...
accs_boot_age_split <- accs_summarized_age_split |>
  partition(cluster) |>
  mutate(icc = map(summary_data, run_icc_bootstrap)) |>
  collect() |>
  select(-summary_data) |>
...
saveRDS(accs_boot_age_split, "../cached_intermediates/1_acc_icc_boot_byage_split.rds")
