...
rt_iccs <- d_rt_dt |>
  ...
  select(-count) |>
  group_by(dataset_name, time_0, min_rt, max_rt, time_end, during, frac, measure, min_trial) |>
  nest() |>
...
rt_iccs_age <- d_rt_dt_byage |>
  ...
  filter(count >= min_trial) |>
  select(-count) |>
  group_by(dataset_name, time_0, min_rt, max_rt, time_end, during, frac, age_bin, measure, min_trial) |>
  nest() |>
  mutate(est = map_dbl(data, \(d) get_icc(d, column = "rt"))) |>
  select(-data)
